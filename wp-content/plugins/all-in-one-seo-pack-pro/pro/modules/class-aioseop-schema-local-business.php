...
if ( ! class_exists( 'AIOSEOP_Schema_Local_Business' ) ) {
	...
	class AIOSEOP_Schema_Local_Business extends All_in_One_SEO_Pack_Module {
		...
		public function registerNotice() {
			...
			$slugs = array(
				'markup'       => 'local_business_markup',
				'organization' => 'local_business_organization',
			);

			// Check if user has a Business or Agency plan.
			if ( ! isset( $aioseop_options['plan'] ) || ! in_array( $aioseop_options['plan'], array( 'business', 'agency' ) ) ) {
				foreach ( $slugs as $k => $v ) {
					$aioseop_notices->remove_notice( $v );
				}
				return;
			}

			if (
				isset( $aioseop_options['modules']['aiosp_feature_manager_options']['aiosp_feature_manager_enable_schema_local_business'] ) &&
				$aioseop_options['modules']['aiosp_feature_manager_options']['aiosp_feature_manager_enable_schema_local_business'] && 
				! $aioseop_options['aiosp_schema_markup']
			) {
				return $aioseop_notices->activate_notice( $slugs['markup'] );
			}
			$aioseop_notices->remove_notice( $slugs['markup'] );

			if ( 'organization' !== $aioseop_options['aiosp_schema_site_represents'] ) {
				return $aioseop_notices->activate_notice( $slugs['organization'] );
			}
			$aioseop_notices->remove_notice( $slugs['organization'] );
		}
